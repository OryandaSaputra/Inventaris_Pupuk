...
const DELIVERY_REVALIDATE_PATHS = [
  ADMIN_ROUTES.home,
  ADMIN_ROUTES.delivery,
...
export async function createAdminDeliveryReceiptAction(
  _prevState: ActionState,
  formData: FormData,
): Promise<ActionState> {
  const { user } = await requireFeatureAccess(
    "canAccessAdminDelivery",
    "Tidak memiliki akses input penerimaan pupuk untuk admin.",
  );
  ...
  try {
    ...
    revalidateMany(DELIVERY_REVALIDATE_PATHS);
