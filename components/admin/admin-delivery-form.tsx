...
import { createAdminDeliveryReceiptAction } from "@/lib/actions/admin-delivery";
...
import type { AdminDeliveryOption } from "@/lib/data/admin-delivery";
...
export function AdminDeliveryForm({
  ...
  const [state, formAction, pending] = useActionState(
    createAdminDeliveryReceiptAction,
    initialActionState,
  );
  ...
  const gardens = useMemo(() => {
    ...
    for (const item of options) {
      map.set(item.gardenId, item.gardenName);
    }
  ...
  const fertilizers = useMemo(() => {
    const map = new Map<string, string>();
    for (const item of options) {
      if (!gardenId || item.gardenId === gardenId) {
  ...
  const supplyOrders = useMemo(() => {
    return options
      .filter(
        (item) =>
          (!gardenId || item.gardenId === gardenId) &&
          (!fertilizerTypeId || item.fertilizerTypeId === fertilizerTypeId),
      )
      .map((item) => ({
        value: item.id,
        label: `${item.sp2bjNumber} · Sisa ${formatNumber(item.remainingQuantity)}`,
      }));
  }, [options, gardenId, fertilizerTypeId]);

  const selectedOrder = useMemo(() => {
    return options.find((item) => item.id === supplyOrderId);
  }, [options, supplyOrderId]);

  const activeGardenCount = useMemo(() => {
    return new Set(options.map((item) => item.gardenId)).size;
  }, [options]);
        ...
        <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
          <div className="min-w-0">
            ...
            <p className="mt-2 max-w-3xl text-sm leading-7 text-slate-600">
              Sebagai admin, Anda dapat menginput penerimaan pupuk untuk semua kebun
              tanpa batasan. Pilih kebun, jenis pupuk, dan SP2BJ yang sesuai, kemudian
              lengkapi detail penerimaan.
            </p>
          </div>

          <div className="grid gap-2 sm:grid-cols-3 xl:min-w-[390px]">
            <MetricChip
              label="Kebun Tersedia"
      ...
      <CardContent className="pt-6">
        <form ref={formRef} action={formAction} className="space-y-6">
          <SectionCard
            ...
            title="Pilih kebun dan kontrak yang akan diterima"
            description="Pilih kebun terlebih dahulu, lalu pilih jenis pupuk dan SP2BJ yang masih memiliki sisa kuantitas. Admin dapat memilih dari seluruh kebun yang terdaftar."
            icon={ClipboardCheck}
          >
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-12">
              <div className="xl:col-span-4">
                <DeliveryFormSelect
                  ...
                  description="Pilih kebun tujuan penerimaan. Semua kebun aktif tersedia."
                  onChange={(event) => {
              ...
              <div className="xl:col-span-4">
                <DeliveryFormSelect
                  ...
                  options={fertilizers}
                  disabled={!gardenId}
                  description="Jenis pupuk akan menyesuaikan kebun yang dipilih."
                  onChange={(event) => {
              ...
              <div className="xl:col-span-4">
                <DeliveryFormSelect
                  ...
                  description="SP2BJ yang masih memiliki sisa volume kontrak."
                ...
                <div className="rounded-[1.6rem] border border-white/70 bg-white/68 p-4 shadow-[0_18px_30px_-26px_rgba(15,56,45,0.2)]">
                  ...
                  {selectedOrder ? (
                    <div className="mt-3 grid gap-3 sm:grid-cols-3">
                      ...
                      <MetricChip
                        label="Kebun"
                        value={selectedOrder.gardenName}
                        tone="info"
                      />
                    </div>
                  ) : (
                    <p className="mt-3 text-sm leading-6 text-slate-600">
                      Pilih kebun, jenis pupuk, dan SP2BJ untuk melihat ringkasan
                      kontrak yang akan dipakai saat input penerimaan.
                    </p>
                  )}
                </div>
              </div>
          ...
          >
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-3">
              <MetricChip
                label="Mode Input"
                value="Admin – Semua Kebun"
                tone="info"
              />
              <MetricChip
                label="Kebun Dipilih"
                value={selectedOrder?.gardenName ?? "Belum dipilih"}
                tone="default"
              />
              <MetricChip
                label="Status Form"
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm leading-6 text-slate-600">
                      Pastikan kebun, SP2BJ, dan volume kiriman sudah benar sebelum
                      data disimpan ke sistem.
                    </p>
                  </div>
                </div>
